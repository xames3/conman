...
.. code-block:: python
    ...
    logger = create_logger(name='app.main', level=logging.INFO)
...
.. versionchanged:: 1.0.0
    ``basic_config`` is now ``create_logger``.
...
def get_logger(module: str) -> logging.Logger:
    ...
    a child logger. Whereas the ``create_logger`` is an application
    level logger.
...
def create_logger(
    ...
    .. versionchanged:: 1.0.0
        The function name is changed from ``basic_config`` to
        ``create_logger`` for more sensibility.
